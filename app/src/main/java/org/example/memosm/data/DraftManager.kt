...
class DraftManager(private val context: Context) {
        ...
        try {
            ...
            while (reader.hasNext()) {
                when (reader.nextName()) {
                    ...
                    "createTime" -> createTime = if (reader.peek() == JsonToken.NULL) { reader.nextNull(); null } else reader.nextString()
                    "filename" -> filename = reader.nextString()
                    "content" -> content = if (reader.peek() == JsonToken.NULL) { reader.nextNull(); null } else reader.nextString()
                    "externalLink" -> externalLink = if (reader.peek() == JsonToken.NULL) { reader.nextNull(); null } else reader.nextString()
                    "type" -> type = reader.nextString()
                    "mimeType" -> mimeType = if (reader.peek() == JsonToken.NULL) { reader.nextNull(); null } else reader.nextString()
                    "size" -> size = if (reader.peek() == JsonToken.NULL) { reader.nextNull(); null } else reader.nextString()
                    "memo" -> memo = if (reader.peek() == JsonToken.NULL) { reader.nextNull(); null } else reader.nextString()
                    else -> reader.skipValue()
                }
            }
            reader.endObject()
            
            return Attachment(name, createTime, filename, content, externalLink, type, mimeType, size, memo)
        } catch (e: Exception) {
    ...
    private fun parseLocation(reader: JsonReader): Location? {
        ...
        try {
            ...
            while (reader.hasNext()) {
                when (reader.nextName()) {
                    "placeholder" -> placeholder = if (reader.peek() == JsonToken.NULL) { reader.nextNull(); null } else reader.nextString()
                    "latitude" -> latitude = if (reader.peek() == JsonToken.NULL) { reader.nextNull(); null } else reader.nextDouble()
                    "longitude" -> longitude = if (reader.peek() == JsonToken.NULL) { reader.nextNull(); null } else reader.nextDouble()
                    else -> reader.skipValue()
                }
            }
            reader.endObject()
