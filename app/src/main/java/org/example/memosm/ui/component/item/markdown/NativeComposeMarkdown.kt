    ...
    val references = remember(tree, content) {
        ...
        tree.children.forEach { child ->
             if (child.type == org.intellij.markdown.MarkdownElementTypes.LINK_DEFINITION) {
                 val labelNode = child.findChildOfType(org.intellij.markdown.MarkdownElementTypes.LINK_LABEL)
                 val destNode = child.findChildOfType(org.intellij.markdown.MarkdownElementTypes.LINK_DESTINATION)
    ...
    CompositionLocalProvider(
        ...
        if (selectable) {
            androidx.compose.foundation.text.selection.SelectionContainer {
                NativeMarkdownNode(
                    node = tree, content = content, modifier = modifier, onContentChange = onContentChange, onHashtagClick = onHashtagClick
                )
            }
        } else {
            NativeMarkdownNode(
